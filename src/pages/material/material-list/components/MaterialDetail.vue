            ...
            <card-body class="q-pt-none">
              <div class="row q-col-gutter-x-md q-col-gutter-y-sm">
                <div class="col-md-6 col-sm-12 col-xs-12">
                  <date-input v-model="formData.date" class="w-full" label="日期 *" placeholder="請選擇日期"
              ...
              <div class="row q-col-gutter-x-md q-col-gutter-y-sm">
                <div class="col-12">
                  <base-button v-for="item in materialClassificationData" :label="item.name" :outline="true"
                    :rounded="true" @click="addNewRow(item)" class="classification-btn" />
                </div>
              </div>
            </card-body>
          </q-card>
        ...
        <div class="col-12">
          <q-card class="h-full shadow-7">
            ...
            <card-body class="q-pt-none">
              <div class="row q-col-gutter-x-md q-col-gutter-y-sm">
                <div class="col-12">
                  <vxe-server-table ref="dataTable" :data="data" :total="total" :current="search.page"
                    @sort-change="OnChangeSort" @update:current="onChangePage">
...
const onDelete = async (rowIndex) => {
  ...
  const res = await messageDelete({ title: '刪除', message: '確認刪除原物料？', confirmButtonText: '確認', cancelButtonText: '取消' })
  if (!res) return
...
const { dataTable, search, data, total, onChangePage, OnChangeSort } = useVxeServerDataTable({
...
const { form, callReadFetch, callCreateFetch, callUpdateFetch, callDeleteFetch } = useCRUD({
  readFetch: readFetch,
