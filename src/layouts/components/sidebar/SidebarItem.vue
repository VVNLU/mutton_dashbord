...
const activeItem = (currentItem) => {
    ...
    open.value = item.value.group ? item.value.group.filter(groupItem => selectMatchSidebarItem([currentItem.group], groupItem).length > 0).length > 0 : false
}
...
const changeActiveHeaderStyle = (currentItem) => {
    ...
    const isGroup = currentItem.group && item.value.group && item.value.group.filter(groupItem => selectMatchSidebarItem([currentItem.group], groupItem).length > 0).length > 0
    if (!isGroup) {
